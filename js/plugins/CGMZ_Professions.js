 ...
 * - Warn instead of crash when detecting most invalid JSON in parameters
 *
 * Version 1.4.1
 ...
 * @option Recipes
 * @value Recipes
 * @option Unlearned Recipes Header
 * @value Unlearned Recipes Header
 * @option Unlearned Recipes
 * @value Unlearned Recipes
...
CGMZ.Professions.UnlearnedRecipeHeaderText = CGMZ.Professions.parameters["Unlearned Recipe Header Text"];
	...
	for(const infoType of CGMZ.Professions.DisplayInfo) {
		switch(infoType) {
			...
			case "Description":
				...
				break;
			case "Custom Info":
				const info = professionTemp._customInfo[customInfoCount++];
				if(info) {
					this._neededHeight += this.CGMZ_drawText(info, 0, 0, this._neededHeight, this.contents.width, 'left');
				}
				break;
			case "Custom Header":
				const header = professionTemp._customHeaders[customHeaderCount++];
				if(header) {
					this.CGMZ_drawHeader(header, this._neededHeight, CGMZ.Professions.HeaderColor1, CGMZ.Professions.HeaderColor2);
					this._neededHeight += this.lineHeight();
				}
				break;
			case "Recipes Header":
			...
			case "Recipes":
				...
				break;
			case "Unlearned Recipes Header":
				if(Imported.CGMZ_Crafting) {
					this.CGMZ_drawHeader(CGMZ.Professions.UnlearnedRecipeHeaderText, this._neededHeight, CGMZ.Professions.HeaderColor1, CGMZ.Professions.HeaderColor2);
					this._neededHeight += this.lineHeight();
				}
				break;
			case "Unlearned Recipes":
				if(Imported.CGMZ_Crafting) {
					const allRecipes = $cgmz.getAllRecipes();
					const recipeType = this._profession.getId();
					const undiscoveredRecipes = [];
					for(const recipe of allRecipes) {
						if(!recipe._discovered && recipe._profession === recipeType) undiscoveredRecipes.push(recipe);
					}
					if(undiscoveredRecipes.length > 0) {
						this.drawProfessionRecipes(undiscoveredRecipes);
					} else {
						this.CGMZ_drawTextLine(CGMZ.Professions.NoRecipesText, 0, this._neededHeight, this.contents.width, 'left');
						this._neededHeight += this.lineHeight();
					}
...
CGMZ_Window_ProfessionDisplay.prototype.drawLargeIcon = function() {
	const profTemp = $cgmzTemp.getProfessionTempData(this._profession._name);
	const iconIndex = profTemp._iconIndex;
